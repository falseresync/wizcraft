...
public class ChargeManager {
        ...
        WAND_CHARGE_SPENT.register((wandStack, cost, user) -> {
            if (user != null) {
                ...
                if (newShells != null) {
                    ...
                    wandStack.update(WizcraftComponents.WAND_CHARGE, 0, it -> it + compensation);
                }
            }
        });
    ...
    public boolean areShellsFull(Player player) {
    ...
    public void applyShellCharge(Player player, int amount) {
    ...
    public boolean cannotAddAnyCharge(ItemStack wandStack, Player player) {
        return isWandFullyCharged(wandStack) && areShellsFull(player);
    }

    public boolean tryExpendWandCharge(ItemStack wandStack, int cost, @Nullable Player user) {
        ...
        if (charge >= cost) {
            wandStack.update(WizcraftComponents.WAND_CHARGE, charge, current -> current - cost);
    ...
    public void chargeWand(ItemStack wandStack, int amount, @Nullable Player user) {
        ...
        wandStack.update(WizcraftComponents.WAND_CHARGE, 0, it -> Math.min(it + amount, max));
        if (current + amount > max) {
            ChargeManager.WAND_OVERCHARGED.invoker().onWandOvercharged(wandStack, current + amount - max, user);
        }
    }

    public void tryChargeWandPassively(ItemStack wandStack, Level world, Player player) {
        ...
        var usageCoefficient = ItemStack.matches(player.getMainHandItem(), wandStack) ? 1f : 0.25f;
    ...
    public float calculateEnvironmentCoefficient(Level world, Player player) {
        var environmentCoefficient = 1f;
        var worldType = world.dimension();
        if (worldType == Level.NETHER) {
            environmentCoefficient *= 0.1f;
        } else if (worldType == Level.END) {
            environmentCoefficient *= 3f;
        } else {
            environmentCoefficient *= world.isNight() ? 1 : 0.5f;
            environmentCoefficient *= world.getBiome(player.blockPosition()).value().hasPrecipitation() ? 1 - world.getRainLevel(1) : 1;
            environmentCoefficient *= world.getBrightness(LightLayer.SKY, player.blockPosition()) / (world.getMaxLightLevel() * 0.5f);
        }
    ...
    public interface WandExpend {
        void onWandChargeSpent(ItemStack wandStack, int cost, @Nullable Player user);
    }

    @FunctionalInterface
    public interface WandOvercharge {
        void onWandOvercharged(ItemStack wandStack, int excess, @Nullable Player user);
    }
}
