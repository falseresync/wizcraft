...
public class MagicCauldronBlockEntity extends BlockEntity implements SidedStorageBlockEntity {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, MagicCauldronBlockEntity entity) {
        ...
        if (!entity.fluidStorage.getResource().isOf(Fluids.WATER)) return;

        var stack = entity.inventory.getStack(0);
        if (stack.isEmpty()) return;

        var recipes = world.getRecipeManager().getAllMatches(WizRecipes.CATALYTIC_CONDENSATION, entity.inventory, world);
        if (recipes.size() == 0) {
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.put(WizNbtConstants.ITEM_STORAGE, inventory.toNbtList());
    }
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        inventory.readNbtList(nbt.getList(WizNbtConstants.ITEM_STORAGE, NbtElement.COMPOUND_TYPE));
    }
