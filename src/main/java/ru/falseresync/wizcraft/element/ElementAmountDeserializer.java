...
public class ElementAmountDeserializer implements JsonDeserializer<ElementAmount> {
    ...
    public ElementAmount deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        var elementAmountJson = JsonHelper.asObject(json, WizJsonConstants.ELEMENT_AMOUNT);
        return new ElementAmount(
                WizRegistries.ELEMENT.get(new Identifier(JsonHelper.getString(elementAmountJson, WizJsonConstants.ELEMENT))),
                JsonHelper.getLong(elementAmountJson, WizJsonConstants.AMOUNT)
        );
    }
}
